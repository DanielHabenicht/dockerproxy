...
commander.name('dockerproxy up')
    ...
    .option('-n, --network <name>',
            'docker network interface that should be proxied')
...
startProxy(commander.address || config.proxyAddress,
...
function startProxy(address, port, name, network) {
  ...
  if (network === '') {
    command = `docker run --rm --name ${
        name} --privileged=true --net=host -e DOCKER_NET -d ncarlier/redsocks ${
        address} ${port}`;
  } else {
    command = `docker run --rm --name ${
        name} --privileged=true --net=host -e DOCKER_NET=${
        network} -d ncarlier/redsocks ${address} ${port}`;
  }
  console.log(command);
  cmd.run(command, function(err, data, stderr) {
    ...
    common.success(`Started Proxy for network "${
        network === '' ? 'ALL' : network}" to "${address}" with id: ${data}`);
    process.exit(0);
  });
}
